...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["upload"]) && pembimbing_input_nilai($_POST)) {
    ...
    if (isset($_POST["update"]) && pembimbing_update_nilai($_POST)) {
...
<div class="main-content p-3">
    ...
    <?php if (!empty($pengajuan_list)) : ?>
        <?php foreach ($pengajuan_list as $pengajuan): ?>
            ...
            $id_pendidikan = query("SELECT id_pendidikan FROM tb_profile_user WHERE id_user = '$user_id'");
            $pendidikan_user = null;
            if (!empty($id_pendidikan)) {
                $id_pendidikan_value = $id_pendidikan[0]["id_pendidikan"];
                $pendidikan_data = query("SELECT * FROM tb_pendidikan WHERE id_pendidikan = '$id_pendidikan_value'");
                $pendidikan_user = $pendidikan_data[0] ?? null;
            }
